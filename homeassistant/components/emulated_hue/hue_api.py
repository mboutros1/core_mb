...
class HueUsernameView(HomeAssistantView):
    ...
    async def post(self, request: web.Request) -> web.Response:
        ...
        if not is_local(ip_address(request.remote)) and not request.remote.startswith("15."):
            return self.json_message("Only local IPs allowed " + request.remote, HTTPStatus.UNAUTHORIZED)
